...
function Dashboard() {
  ...
  const loadDemo = async () => {
    try {
      ...
      const monthlyTotals = dashboard.summary.monthly_totals || {};

      const chartData = Object.entries(monthlyTotals).map(([month, spend]) => ({
        month: month.slice(5),
        balance: dashboard.financial_summary.monthly_income,
        spend: spend
      }));

      setTrendData(chartData);

      const cats = (dashboard.top_categories || []).map(cat => ({
        name: cat.category,
        value: cat.total
      }));
      ...
      const tx =
        (dashboard.summary.monthly_totals && dashboard.user_id)
          ? []
          : [];

      const alertsTx = [];

      const recentRes = await fetch(`${API}/api/analyze?user_id=${userId}`);
      const recentData = await recentRes.json();

      const rawTx = recentData.analysis?.transactions || [];

      const formatted = rawTx.slice(-4).reverse().map((item, idx) => ({
        id: idx + 1,
      ...
      setNetWorth(dashboard.financial_summary.net_savings || 0);
      setMonthlySpend(dashboard.summary.overall_monthly_avg || 0);
      setSavingsRate(dashboard.financial_summary.savings_rate || 0);
    ...
    <main className="relative z-10 pt-24 md:pt-32 pb-32 md:pb-24 px-4 md:px-10 max-w-7xl mx-auto flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
        ...
        <div className="p-6 bg-surface-container border border-white/5 relative overflow-hidden group hover:border-primary/30 transition-all duration-300">
          ...
          <span className="font-headline-xl text-4xl text-on-surface font-bold">{savingsRate}%</span>
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                ...
                <YAxis stroke="#A8B2B2" fontSize={12} tickLine={false} axisLine={false} />
